...
import { toggleCartModal } from "../../Features/Slices/CartSlice";
...
function Navbar({ inHeader, showMenu, setShowMenu }: Props) {
   const show: boolean = useAppSelector((store) => store.cart.showCart);
   const [showModal, setShowModal] = useState(show);
   const { checkout } = useParams();
   const dispatch = useAppDispatch();

   const handleMenu = () => {
      setShowMenu && setShowMenu((prev: boolean) => !prev);
   };

   const handleToggle = () => {
      !checkout && !showMenu && setShowModal((prev) => !prev);
   };
   useEffect(() => {
      show != showModal && setShowModal(show);
   }, [show]);

   useEffect(() => {
      show != showModal && dispatch(toggleCartModal(showModal));
   }, [showModal]);
   return (
      <div
         className={`flex justify-between uppercase w-full items-center ${
            !inHeader
               ? "flex-col mt-12 md:flex-wrap md:flex-row md:mt-14"
               : "flex-row "
         }`}
      >
         <div className={`${!inHeader && "hidden"} lg:hidden relative`}>
            <svg
               width="16"
               height="15"
         ...
         <nav
            className={`text-white ${
               inHeader
                  ? "hidden space-x-[2.37vw]"
                  : "flex flex-col mt-12 gap-y-4 items-center md:gap-x-[34px] md:w-full md:gap-y-0 lg:w-fit lg:mt-0"
            } lg:flex md:flex-row tracking-[2px]`}
         >
         ...
         {inHeader && (
            <span
               ...
               className={`${!checkout && "hover:cursor-pointer"}`}
            >
               <svg
                  width="23"
                  height="20"
                  xmlns="http://www.w3.org/2000/svg"
                  className={`fill-white hover:fill-primary hover:cursor-pointer ${
                     checkout &&
                     "hover:cursor-auto hover:fill-none fill-transparent"
                  }`}
               >
                  <path
